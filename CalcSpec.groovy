...
class CalcSpec extends Specification {
     ...
     * Works.
     */
    void "simple test, add a number and a list of numbers (parallel)"() {
        setup:
        calc = new Calc()

        expect:
        calc.add(2) == 2
        calc.addListParallel([-5, 4, 8]) == 9
    }
     ...
     */
    @Ignore
    void "Test with calc as spy, add a 2x number and a list of 2x numbers (parallel)"() {
        setup:
        calc = Spy(Calc) {
            add(_) >> { Integer n ->
                calc.res.addAndGet(2 * n)
